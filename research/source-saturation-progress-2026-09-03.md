...
- Preserved the complete 128-record public AMR Substack archive plus five RSS-only podcast companion items, the complete 31-video YouTube Videos-tab inventory, public player metadata, and an English caption VTT for every captured video. The exact-title cross-platform map identifies 112 website/Substack/YouTube groups without forcing differently titled items together. Reviewed official titles, descriptions, and opening captions establish 83 participant appearances across 30 videos and 40 distinct printed names; the remaining video has no participant names established by those bounded fields. The importer remains a local/manual source-capture tool; no network-fetch workflow is enabled in GitHub Actions.
...
- Added a conservative identity-resolution layer covering 2,799 source rows across 28 datasets. It confirms 672 row mappings to 206 canonical people, adds 189 recurring source-backed person nodes, and leaves 2,127 rows unresolved rather than forcing matches. The 823-row review queue preserves initials, duplicates, common first-name/full-name variants, and collisions. The process also corrected the pre-existing mistaken Andrew Augenstein ID on the 2022 A Faithful PCA row for Rev. Steve Brown.
- Produced the first source-backed overlap analysis: 378 pairwise comparisons, 204 people recurring across at least two independent source families, 559 presbytery concentration rows, 208 exact current-directory church matches, six dated institution/action pipeline people, and 672 confirmed identity-to-dataset graph edges. Whole-roster lower bounds and confirmed-subset percentages are reported separately.
