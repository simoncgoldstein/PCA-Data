...
# Patch the identity builder with source-specific matching forms and new datasets.
bp = root / 'scripts/build-person-crosswalk.py'
text = bp.read_text(encoding='utf-8')

sig_old = '''    evidence_type: str,
    presbyteries: list[str] | None = None,
'''
sig_new = '''    evidence_type: str,
    matching_name: str | None = None,
    presbyteries: list[str] | None = None,
'''
if 'matching_name: str | None = None' not in text:
    if sig_old not in text:
        raise SystemExit('builder matching_name signature anchor missing')
    text = text.replace(sig_old, sig_new, 1)

candidate_old = '''    printed_candidate = normalize_name(printed_name)
...
'''
candidate_new = '''    printed_candidate = normalize_name(printed_name)
...
'''
if 'matching_candidate = normalize_name(matching_name)' not in text:
    if candidate_old not in text:
        raise SystemExit('builder candidate anchor missing')
    text = text.replace(candidate_old, candidate_new, 1)

public_old = '''        "normalized_candidate": candidate,
        "normalized_name_as_printed": printed_candidate,
'''
public_new = '''        "normalized_candidate": candidate,
        "normalized_name_as_printed": printed_candidate,
        "normalized_matching_name": matching_candidate,
'''
if '"normalized_matching_name": matching_candidate' not in text:
    if public_old not in text:
        raise SystemExit('builder public-record anchor missing')
    text = text.replace(public_old, public_new, 1)

builder_marker = '# 2003 Good Faith Subscription person-level evidence.'
if builder_marker not in text:
    anchor = '# 2001-2002 Women in the Military study-committee evidence. All committee,\n'
    if anchor not in text:
        raise SystemExit('builder insertion anchor missing')
    snippet = '''# 2003 Good Faith Subscription person-level evidence. Every action below is
...
# Patch the formal-position validator.
vp = root / 'scripts/validate-formal-position-evidence.py'
v = vp.read_text(encoding='utf-8')

if 'subscription2003_path = root /' not in v:
    load_anchor = 'women2016 = json.loads(women2016_path.read_text(encoding="utf-8"))\n'
    load_new = load_anchor + '''subscription2003_path = root / "sources/normalized/general-assembly/2003-subscription-formal-position-records.json"
...
'''
    ...
    v = v.replace(load_anchor, load_new, 1)

validator_marker = '# 2002-2007 Good Faith Subscription / Federal Vision formal-position family.'
if validator_marker not in v:
    check_anchor = '# 2001-2002 Women in the Military report family.\n'
    checks = '''# 2002-2007 Good Faith Subscription / Federal Vision formal-position family.
...
if '"2007-fv-npp-unanimous-study-report",' not in v:
    required_anchor = '    "2001-wim-consensus-report",\n'
    required_new = '''    "2003-bco-21-4-recorded-negative-votes",
