...
(async()=>{
 ...
 await check('overview',/Follow the evidence/);await page.screenshot({path:path.join(out,'overview-desktop.png'),fullPage:true});
 ...
 await page.getByLabel('Confidence',{exact:true}).selectOption('confirmed');await page.getByRole('button',{name:'Apply',exact:true}).click();await page.waitForURL(/confidence=confirmed/);assert.equal(await page.locator('[data-canonical-score]').innerText(),score);assert.match(await text(),/Index unchanged/);
